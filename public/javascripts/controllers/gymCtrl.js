pokeApp.controller('gymCtrl', ['$scope', '$http', '$q', 'UsersAPI', 'BattleFactory', 'PokemonAPI', function($scope, $http, $q, UsersAPI, BattleFactory, PokemonAPI) {
  ...
  $scope.initializeMatch = function() {
    UsersAPI.getMe().success(function(data){
      ...
      UsersAPI.getGymLeader().success(function(data) {
        $scope.rival = data
        ...
        $scope.spoils = {
          xp: 10 + Math.round(2 + $scope.githubUser.storyProgress/2),
